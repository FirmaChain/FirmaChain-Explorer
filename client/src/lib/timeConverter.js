...
const timeConverter = (time, showFull) => {
  if(showFull) {
    ...
    if(showFull)
      str += ` ${hours}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
  ...
  if (timeString.toString().length === 13)
    timeString = Math.floor(timeString / 1000);
  ...
  switch (true) {
    ...
    case (timeGap < 60 * 60 * 24):
      return {id: 'hoursAgo', values: {num: Math.floor(timeGap / 60 / 60)}};
