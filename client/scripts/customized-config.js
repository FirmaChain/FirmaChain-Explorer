// import codes from https://daveceddia.com/customize-create-react-app-webpack-without-ejecting/
/*
...
const config = configDir + '/config-overrides';
switch(process.argv[2]) {
  // The "start" script is run during development mode
  case 'start':
    rewireModule('react-scripts/scripts/start.js', loadCustomizer(config));
    break;
  // The "build" script is run to produce a production bundle
  case 'build':
    rewireModule('react-scripts/scripts/build.js', loadCustomizer(config));
    break;
